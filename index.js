  ...
  try {
    const response = await fetch("https://prince.services/api/v2", {
      ...
      body: new URLSearchParams({
        key: process.env.Moreimport express from "express";
import fetch from "node-fetch";
import cors from "cors";
...
// صفحة افتراضية
app.get("/", (req, res) => {
  res.send("✅ tunisie Proxy API is running on Render!");
});
...
// صفحة افتراضية
app.get("/", (req, res) => {
  res.send("✅ More Proxy API is running on Render!");
});
...
// تشغيل السيرفر
const PORT = process.env.PORT || 3000;
app.listen(PORT, () => console.log(`🚀 Server running on port ${PORT}`));_API_KEY,
        action: "services"
      })
    });
    const data = await response.json();
